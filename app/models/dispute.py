...
class Dispute(Base):
    ...
    raised_by: Mapped[str] = mapped_column(sa.String(10), nullable=False)  # buyer|seller
    reason: Mapped[str | None] = mapped_column(sa.Text)
    status: Mapped[str] = mapped_column(sa.String(20), nullable=False)     # open|resolved|rejected
    created_at: Mapped[str] = mapped_column(sa.DateTime(timezone=True), server_default=func.now(), nullable=False)
    resolved_at: Mapped[str | None] = mapped_column(sa.DateTime(timezone=True))
